...
import { handle } from '@/lib/chat/handlers/completions'
...
describe('completions handler', () => {
  ...
  it('streams when enabled', async () => {
    ...
    const req: any = { body: { messages: [], model: 'gpt-4o', stream: true } }
    const res = createRes()
  ...
  it('returns json when streaming disabled', async () => {
    mockCall.mockResolvedValue({ ok: true })
    const req: any = { body: { messages: [], model: 'gpt-4o', stream: false } }
    const res = createRes()
    await handle(req, res, { requestId: 'r', signal: new AbortController().signal, startTime: 0, isConversational: false })
  ...
  it('passes requestId on error', async () => {
    ...
    expect(mockError).toHaveBeenCalledWith(res, 500, 'ERR', 'oops', 'req-1')
  })
})
